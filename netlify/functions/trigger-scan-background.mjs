...
function parseParlayAndLock(parlayText) {
  ...
  for (var i = 0; i < lines.length; i++) {
    var line = lines[i].trim();
    var clean = line.replace(/\*\*/g, "").replace(/##/g, "").replace(/🔒/g, "").replace(/🔐/g, "").trim();
    var lower = clean.toLowerCase();
    if (lower.indexOf("lock of the day") >= 0) {
      var lockText = clean.replace(/lock of the day:?/i, "").trim();
      if (lockText.length > 5) {
        result.lock = lockText;
      } else {
        for (var j = i + 1; j < lines.length && j < i + 4; j++) {
          var nextLine = lines[j].replace(/\*\*/g, "").replace(/##/g, "").replace(/🔒/g, "").replace(/^[-•*]\s*/, "").trim();
          if (nextLine.length > 5 && nextLine.indexOf("---") < 0 && nextLine.toLowerCase().indexOf("bet responsib") < 0) {
            result.lock = nextLine;
            break;
          }
        }
      }
    ...
    if (inParlay) {
      ...
      var legLower = legText.toLowerCase();
      if (legText.length > 5 && legText.length < 200 && legLower.indexOf("combined") < 0 && legLower.indexOf("total odds") < 0 && legLower.indexOf("parlay odds") < 0 && legLower.indexOf("estimated") < 0 && legLower.indexOf("payout") < 0) {
        var dashIdx = legText.indexOf(" — ");
        if (dashIdx > 0) legText = legText.substring(0, dashIdx).trim();
