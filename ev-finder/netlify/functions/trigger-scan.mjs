...
async function getGameOdds(apiKey, sportKey) {
  ...
  return apiFetch(`${ODDS_API_BASE}/sports/${sportKey}/odds/?apiKey=${apiKey}®ions=us,us2&markets=${GAME_MARKETS.join(",")}&oddsFormat=american&bookmakers=${books}`);
}

async function getProps(apiKey, sportKey, eventId) {
  const books = [...SHARP_BOOKS, ...SOFT_BOOKS].join(",");
  return apiFetch(`${ODDS_API_BASE}/sports/${sportKey}/events/${eventId}/odds?apiKey=${apiKey}®ions=us,us2&markets=${PROP_MARKETS.join(",")}&oddsFormat=american&bookmakers=${books}`);
}
    ...
    for (const sm of bmMap[sharpKey].markets || []) {
      ...
      for (const sk of SOFT_BOOKS) {
        ...
        for (const out of sMarket.outcomes) {
          ...
          bets.push({ sport: sportLabel, icon: sportIcon, type: betType, marketLabel: mktLabel, game, pick, bookName: BOOK_NAMES[sk] || sk, bookOdds: out.price, sharpOdds: sharp.sp, trueProb: sharp.tp, ev, edge: sharp.tp - toImplied(out.price) });
        }
      }
    }
  }
...
function buildDiscordEmbeds(bets) {
  ...
  const summaryEmbed = { title: "📈 +EV Best Bets", description: today, color: 0x00e676, fields: [{ name: "Opportunities", value: `${bets.length}`, inline: true },{ name: "Avg EV", value: fmtEV(avgEV), inline: true },{ name: "Top EV", value: fmtEV(topEV), inline: true }], footer: { text: "Sharp: Pinnacle (de-vigged) · Gamble responsibly" }, timestamp: new Date().toISOString() };
  const betEmbeds = [];
...
async function sendToDiscord(webhookUrl, bets) {
  ...
  for (let i = 0; i < embeds.length; i += 10) {
    ...
    if (!res.ok) { const body = await res.text(); throw new Error(`Discord: ${res.status} ${body}`); }
...
async function runScan() {
  ...
  for (const sport of SPORTS) {
    let events;
    try { events = await getGameOdds(ODDS_API_KEY, sport.key); }
    catch (e) { if (e.message.includes("422")) continue; continue; }
    if (!events?.length) continue;
    allBets.push(...extractEV(events, sport.label, sport.icon));
    const withSharp = events.filter((e) => e.bookmakers?.some((b) => SHARP_BOOKS.includes(b.key))).slice(0, 4);
    for (const ev of withSharp) {
      try { const pd = await getProps(ODDS_API_KEY, sport.key, ev.id); if (pd?.bookmakers) allBets.push(...extractEV([{ ...ev, bookmakers: pd.bookmakers }], sport.label, sport.icon, true)); } catch {}
    }
  }
  const seen = new Set();
  ...
  if (!bets.length) {
    await fetch(DISCORD_WEBHOOK, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ username: "+EV Finder", content: "📭 **No +EV bets found today.** Lines are tight." }) });
    return { statusCode: 200, body: "No +EV bets" };
  }
  await sendToDiscord(DISCORD_WEBHOOK, bets);
  return { statusCode: 200, body: `Sent ${bets.length} +EV bets to Discord` };
}

const handler = schedule("0 14 * * *", async (event) => {
  return await runScan();
});

export { handler };
File 2: trigger-scan.mjs
GitHub path: netlify/functions/trigger-scan.mjs

📋 Copy trigger-scan.mjs
const ODDS_API_BASE = "https://api.the-odds-api.com/v4";
const MIN_EV = 0.01;
const MAX_BETS = 20;
...
export async function handler(event) {
  const ODDS_API_KEY = process.env.ODDS_API_KEY;
  const DISCORD_WEBHOOK = process.env.DISCORD_WEBHOOK;
  if (!ODDS_API_KEY) return { statusCode: 500, body: "Missing ODDS_API_KEY env var" };
  if (!DISCORD_WEBHOOK) return { statusCode: 500, body: "Missing DISCORD_WEBHOOK env var" };
  const allBets = [];
  for (const sport of SPORTS) {
    let events;
    try {
      const books = [...SHARP_BOOKS, ...SOFT_BOOKS].join(",");
      events = await apiFetch(`${ODDS_API_BASE}/sports/${sport.key}/odds/?apiKey=${ODDS_API_KEY}®ions=us,us2&markets=${GAME_MARKETS.join(",")}&oddsFormat=american&bookmakers=${books}`);
    } catch (e) { if (e.message.includes("422")) continue; continue; }
    if (!events?.length) continue;
    allBets.push(...extractEV(events, sport.label, sport.icon));
    const withSharp = events.filter((e) => e.bookmakers?.some((b) => SHARP_BOOKS.includes(b.key))).slice(0, 4);
  ...
  const bets = allBets.filter((b) => { const k = `${b.bookName}|${b.pick}|${b.game}|${b.marketLabel}`; if (seen.has(k)) return false; seen.add(k); return true; }).sort((a, b) => b.ev - a.ev);
  if (!bets.length) {
    await fetch(DISCORD_WEBHOOK, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ username: "+EV Finder", content: "📭 **No +EV bets found.** Lines are tight!" }) });
    return { statusCode: 200, body: "No +EV bets" };
  }
  await sendToDiscord(DISCORD_WEBHOOK, bets);
  return { statusCode: 200, body: `Sent ${bets.length} +EV bets to Discord` };
